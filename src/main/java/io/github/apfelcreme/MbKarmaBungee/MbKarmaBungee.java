...
public class MbKarmaBungee extends Plugin implements Listener {
	 ...
	 * @param seesParticles
	 */
	public void sendEffectChangeMessage(ProxiedPlayer targetPlayer, String effect, long effectDelay, boolean seesParticles) {
	...
	private Configuration createTextNodes(Configuration config) {
		...
		//Broadcast
		// Info: Zeilenumbrüche sind mit ChatColors buggy, sie springen wieder auf weiß. deswegen 
		...
		config.set(
				"texts.info.listHead",
				"{LIGHT_PURPLE}Spieler: {WHITE}{0}{LIGHT_PURPLE}, P={WHITE}Player{LIGHT_PURPLE}, R={WHITE}Ratio{LIGHT_PURPLE}, K={WHITE}verg. Karma{LIGHT_PURPLE}, Z={WHITE}Letzte Transakt.");
		config.set(
				"texts.info.list",
				"{LIGHT_PURPLE}P: {WHITE}{0} {LIGHT_PURPLE}R: {WHITE}{1} {LIGHT_PURPLE}K: {WHITE}{2} {LIGHT_PURPLE}Z: {WHITE}{3}");
		...
		config.set("effects.POTION_BREAK.aliases", Arrays.asList("POTION_BREAK", "POTION-BREAK", "POTIONBREAK"));
		config.set("effects.ENDER_SIGNAL.delay", 10L);
		config.set("effects.ENDER_SIGNAL.displaytext", "Endersignal");
		config.set("effects.ENDER_SIGNAL.aliases", Arrays.asList("ENDER_SIGNAL", "ENDERSIGNAL", "ENDER-SIGNAL"));
		config.set("effects.MOBSPAWNER_FLAMES.delay", 10L);
		config.set("effects.MOBSPAWNER_FLAMES.displaytext", "Mobspawner-Flames");
		config.set("effects.MOBSPAWNER_FLAMES.aliases", Arrays.asList("MOBSPAWNER_FLAMES", "MOBSPAWNER-FLAMES", "MOBSPAWNER", "MOBSPAWNERFLAMES"));
		config.set("effects.FIREWORKS_SPARK.delay", 10L);
		config.set("effects.FIREWORKS_SPARK.displaytext", "Fireworks-Spark");
		config.set("effects.FIREWORKS_SPARK.aliases", Arrays.asList("FIREWORKS_SPARK", "FIREWORKS-SPARK", "FIREWORKSSPARK", "FEUERWERK"));
		...
		config.set("effects.MAGIC_CRIT.aliases", Arrays.asList("MAGIC_CRIT", "MAGICCRIT", "MAGIC-CRIT","MAGIC-KRIT","MAGIE-KRIT","MAGIEKRIT"));
		config.set("effects.POTION_SWIRL.delay", 10L);
		config.set("effects.POTION_SWIRL.displaytext", "Potionswirl");
		config.set("effects.POTION_SWIRL.aliases", Arrays.asList("POTION_SWIRL", "POTIONSWIRL", "POTION-SWIRL"));
		config.set("effects.POTION_SWIRL_TRANSPARENT.delay", 10L);
		config.set("effects.POTION_SWIRL_TRANSPARENT.displaytext", "Potionswirl-Transparent");
		config.set("effects.POTION_SWIRL_TRANSPARENT.aliases", Arrays.asList("POTION_SWIRL_TRANSPARENT", "POTION_SWIRL-TRANSPARENT", "POTIONSWIRL-TRANSPARENT", "POTION-SWIRL-TRANSPARENT","POTIONSWIRLTRANSPARENT"));
		...
		config.set("effects.INSTANT_SPELL.aliases", Arrays.asList("INSTANT_SPELL", "INSTANTSPELL", "INSTANT-SPELL", "INSTANT-SPRUCH"));
		config.set("effects.WITCH_MAGIC.delay", 5L);
		config.set("effects.WITCH_MAGIC.displaytext", "Witchmagic");
		config.set("effects.WITCH_MAGIC.aliases", Arrays.asList("WITCH_MAGIC", "WITCHMAGIC", "WITCH-MAGIC", "WITCH", "HEXE", "HEXENMAGIE", "HEXEN-MAGIE","HEXEN-MAGIC"));
		...
		config.set("effects.PORTAL.aliases", Arrays.asList("PORTAL", "NETHERPORTAL","NETHER-PORTAL"));
		config.set("effects.FLYING_GLYPH.delay", 5L);
		config.set("effects.FLYING_GLYPH.displaytext", "Flying-Glyph");
		config.set("effects.FLYING_GLYPH.aliases", Arrays.asList("FLYING_GLYPH", "FLYING-GLYPH", "GLYPH","FLIEGENDER-GLYPH", "GLYPHE"));
		config.set("effects.FLAME.delay", 3L);
		config.set("effects.FLAME.displaytext", "Flame");
		config.set("effects.FLAME.aliases", Arrays.asList("FLAME", "FLAMME","FEUER"));
		config.set("effects.LAVA_POP.delay", 4L);
		config.set("effects.LAVA_POP.displaytext", "Lava-Pop");
		config.set("effects.LAVA_POP.aliases", Arrays.asList("LAVA_POP", "LAVA-POP", "LAVAPOP", "LAVAEXPLOSION"));
		...
		config.set("effects.PARTICLE_SMOKE.aliases", Arrays.asList("PARTICLE_SMOKE", "PARTICLE-SMOKE","PARTIKEL-RAUCH","PARTIKELRAUCH","PARTICLESMOKE"));
		config.set("effects.EXPLOSION_HUGE.delay", 20L);
		config.set("effects.EXPLOSION_HUGE.displaytext", "Explosion-Sehr-Groß");
		config.set("effects.EXPLOSION_HUGE.aliases", Arrays.asList("EXPLOSION_HUGE", "EXPLOSION-SEHR-GROß","EXPLOSION-HUGE"));
		config.set("effects.EXPLOSION_LARGE.delay", 15L);
		config.set("effects.EXPLOSION_LARGE.displaytext", "Explosion-Groß");
		config.set("effects.EXPLOSION_LARGE.aliases", Arrays.asList("EXPLOSION_LARGE", "EXPLOSION-GROß","EXPLOSIONLARGE"));
		config.set("effects.EXPLOSION.delay", 10L);
		config.set("effects.EXPLOSION.displaytext", "Explosion");
		config.set("effects.EXPLOSION.aliases", Arrays.asList("EXPLOSION" ,"EXPLOSION-NORMAL"));
		config.set("effects.VOID_FOG.delay", 1L);
		config.set("effects.VOID_FOG.displaytext", "Void-Fog");
		config.set("effects.VOID_FOG.aliases", Arrays.asList("VOID_FOG", "VOIDFOG","VOID-FOG", "VOIDNEBEL", "VOID"));
		config.set("effects.SMALL_SMOKE.delay", 5L);
		config.set("effects.SMALL_SMOKE.displaytext", "Wenig-Rauch");
		config.set("effects.SMALL_SMOKE.aliases", Arrays.asList("SMALL_SMOKE", "SMALLSMOKE","WENIG-RAUCH","WENIG-SMOKE"));
		...
		config.set("effects.COLOURED_DUST.aliases", Arrays.asList("COLOURED_DUST", "COLOURED-DUST","REDSTONE","FARBSTAUB","COLOUREDDUST"));
		config.set("effects.SNOWBALL_BREAK.delay", 4L);
		config.set("effects.SNOWBALL_BREAK.displaytext", "Schneeball-Break");
		config.set("effects.SNOWBALL_BREAK.aliases", Arrays.asList("SNOWBALL_BREAK", "SCHNEEBALL-BREAK", "SNOWBALL-BREAK", "SCHNEEBALL"));
		config.set("effects.WATERDRIP.delay", 5L);
		config.set("effects.WATERDRIP.displaytext", "Water-Drip");
		config.set("effects.WATERDRIP.aliases", Arrays.asList("WATERDRIP", "WATER-DRIP","WASSERTROPFEN","REGEN","RAIN","WATERDROP"));
		config.set("effects.LAVADRIP.delay", 5L);
		config.set("effects.LAVADRIP.displaytext", "Lava-Drip");
		config.set("effects.LAVADRIP.aliases", Arrays.asList("LAVADRIP", "LAVA-DRIP","LAVADROP"));
		config.set("effects.SNOW_SHOVEL.delay", 5L);
		config.set("effects.SNOW_SHOVEL.displaytext", "Schneeschaufel");
		config.set("effects.SNOW_SHOVEL.aliases", Arrays.asList("SNOW_SHOVEL", "SCHNEESCHAUFEL"));
		...
		config.set("effects.HEART.aliases", Arrays.asList("HEART", "HERZ", "HERZCHEN"));
		config.set("effects.VILLAGER_THUNDERCLOUD.delay", 5L);
		config.set("effects.VILLAGER_THUNDERCLOUD.displaytext", "Villager-Thundercloud");
		config.set("effects.VILLAGER_THUNDERCLOUD.aliases", Arrays.asList("VILLAGER_THUNDERCLOUD", "VILLAGER-THUNDERCLOUD"));
		config.set("effects.HAPPY_VILLAGER.delay", 5L);
		config.set("effects.HAPPY_VILLAGER.displaytext", "Happy-Villager");
		config.set("effects.HAPPY_VILLAGER.aliases", Arrays.asList("HAPPY_VILLAGER", "HAPPY-VILLAGER"));
		config.set("effects.LARGE_SMOKE.delay", 8L);
		config.set("effects.LARGE_SMOKE.displaytext", "Viel-Rauch");
		config.set("effects.LARGE_SMOKE.aliases", Arrays.asList("LARGE_SMOKE", "LARGE-SMOKE", "VIEL-RAUCH"));
